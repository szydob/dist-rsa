...
from core.factorization.worker import factor_chunk
...
class FactorizationCoordinator:
	...
	def factor(self, n: int) -> FactorizationResult:
		...
		self.log.info(
			"Task %s started with %s chunks and %s/%s workers",
			task.task_id,
			len(chunks),
			effective_workers,
			self.workers_count,
		...
		while chunk_queue and len(pending) < effective_workers:
			chunk = chunk_queue.popleft()
			pending[factor_chunk.remote(n, chunk)] = chunk
